...
async function verificarViviendas() {
  ...
  if (viviendas && viviendas.length > 0) {
    ...
    viviendas.forEach((v, i) => {
      console.log(`${i + 1}. Vivienda ${v.manzanas?.nombre || 'N/A'}${v.numero}`)
      console.log(`   ID: ${v.id}`)
    ...
    if (sinNegociacion.length > 0) {
      console.log(`⚠️ ${sinNegociacion.length} vivienda(s) asignada(s) SIN negociación_id:\n`)
      sinNegociacion.forEach(v => {
        console.log(`   - ${v.manzanas?.nombre}${v.numero} (Cliente: ${v.clientes?.nombres} ${v.clientes?.apellidos})`)
      })
    }
  } else {
