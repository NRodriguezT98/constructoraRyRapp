...
async function verificarHistorial() {
  ...
  clientes?.forEach(c =>
    console.log(`   - ${c.nombres} ${c.apellidos} (${c.id})`)
  )
  console.log('')
  ...
  const { data: negociacion } = await supabase
    .from('negociaciones')
    .select(
      'id, version_actual, version_lock, fecha_ultima_modificacion, estado'
    )
  ...
  console.log(
    `   Última Modificación: ${negociacion.fecha_ultima_modificacion}\n`
  )
  ...
  const { data: historial } = await supabase
    ...
    .select(
      'version, tipo_cambio, razon_cambio, fecha_cambio, campos_modificados'
    )
    .eq('negociacion_id', negociacion.id)
    .order('version', { ascending: false })

  console.log(
    `📚 HISTORIAL DE VERSIONES (${historial?.length || 0} snapshots):\n`
  )
  ...
  if (historial && historial.length > 0) {
    historial.forEach((h, i) => {
      ...
      console.log(
        `      Fecha: ${new Date(h.fecha_cambio).toLocaleString('es-CO')}`
      )
  ...
  const { data: fuentes } = await supabase
    .from('fuentes_pago')
    .select(
      'tipo, monto_aprobado, monto_recibido, estado_fuente, fecha_creacion, razon_inactivacion'
    )
    .eq('negociacion_id', negociacion.id)
    .order('fecha_creacion', { ascending: false })
  ...
  if (fuentes && fuentes.length > 0) {
    fuentes.forEach((f, i) => {
      ...
      console.log(
        `      Recibido: $${f.monto_recibido.toLocaleString('es-CO')}`
      )
      console.log(`      Estado: ${f.estado_fuente}`)
      console.log(
        `      Fecha Creación: ${new Date(f.fecha_creacion).toLocaleString('es-CO')}`
      )
