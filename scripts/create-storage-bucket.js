...
async function createBucket() {
  ...
  if (error) {
    ...
    if (error.message.includes('permission') || error.message.includes('JWT')) {
      ...
      console.log('')
      console.log(
        'Este script necesita la SERVICE_ROLE key de Supabase, no la ANON key.'
      )
      console.log('')
      ...
      console.log('   --------------------------------------------------------')
      console.log(
        '   1. Ir a: https://supabase.com/dashboard/project/swyjhwgvkfcfdtemkyad/settings/api'
      )
      console.log('   2. Copiar "service_role" key (⚠️ secreta, no compartir)')
      console.log(
        '   3. Crear archivo .env.local con: SUPABASE_SERVICE_ROLE_KEY=tu-key'
      )
