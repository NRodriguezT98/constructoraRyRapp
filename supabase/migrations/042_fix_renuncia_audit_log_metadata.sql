-- =====================================================
-- MIGRACIÓN 042: Enriquecer metadata en audit_log de renuncias
-- =====================================================
-- PROBLEMA:
--   1. `cliente_id` estaba en `datos_nuevos`, no en `metadata`.
--      El servicio historial-cliente filtra por
--      .contains('metadata', { cliente_id }), por lo que los
--      eventos de renuncia NUNCA aparecían en el historial.
--   2. La metadata era muy escasa (solo evento, proyecto,
--      cliente_nombre, vivienda_numero). Faltaba motivo,
--      montos, retención, manzana, estado de la renuncia.
--
-- FIX:
--   - Mover `cliente_id` a `metadata` (clave crítica).
--   - Agregar: motivo, manzana_nombre, retencion_monto,
--     retencion_motivo, monto_a_devolver, requiere_devolucion,
--     estado_renuncia al metadata.
--   - Mover el SELECT de v_renuncia (post-trigger) antes del
--     INSERT en audit_log para capturar monto_a_devolver
--     calculado automáticamente por el trigger.
-- =====================================================
...
BEGIN
  ...
  -- (necesario para capturar monto_a_devolver calculado)
  -- ═══════════════════════════════════════
  ...
  -- ═══════════════════════════════════════
  -- AUDITORÍA (metadata enriquecida con cliente_id)
  -- ═══════════════════════════════════════
