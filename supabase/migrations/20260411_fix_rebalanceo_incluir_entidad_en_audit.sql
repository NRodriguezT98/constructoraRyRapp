-- ============================================================
-- MIGRACIÓN: Incluir cambio de entidad en audit log del cierre financiero
-- Fecha: 2026-04-11
-- Problema:
--   v_ajustes_aplicados en rebalancear_plan_financiero solo almacenaba
--   monto_anterior/monto_nuevo pero NO entidad_anterior/entidad_nueva.
--   Resultado: el historial mostraba "BBVA → BBVA" cuando el cambio
--   real era "BBVA Colombia → Bancolombia".
-- Solución:
--   Agregar entidad_anterior, entidad_nueva y cambio_entidad al jsonb
--   de v_ajustes_aplicados para el caso 'actualizada'.
-- ============================================================
    ...
    LOOP
      ...
      ELSE
        ...
        IF v_ajuste_monto_nuevo IS DISTINCT FROM v_ajuste_monto_original
          ...
          -- ✅ FIX: Incluir entidad_anterior, entidad_nueva y cambio_entidad
          v_ajustes_aplicados := v_ajustes_aplicados || jsonb_build_object(
            'id',               v_ajuste_id,
            'tipo',             v_ajuste_tipo,
            'accion',           'actualizada',
