...
interface ProyectosStore {
    ...
    actualizarProyecto: (id: string, proyecto: Partial<ProyectoFormData>) => Promise<void>
...
export const useProyectosStore = create<ProyectosStore>((set, get) => ({
    ...
    actualizarProyecto: async (id: string, proyectoActualizado: Partial<ProyectoFormData>) => {
        set({ isLoading: true, error: null })
        try {
            const proyecto = await proyectosService.actualizarProyecto(id, proyectoActualizado)
            set((state) => ({
