...
export function DocumentoEditModal({
  ...
  useEffect(() => {
    if (isOpen && documento) {
      ...
      setNumeroResolucion(String(documento.metadata?.numero_resolucion || ''))
      setFechaResolucion(String(documento.metadata?.fecha_resolucion || ''))
      setError('')
    }
  }, [isOpen, documento])
  ...
  const esSubsidio =
    (typeof documento.metadata?.tipo_fuente === 'string' &&
      documento.metadata.tipo_fuente.includes('Subsidio')) ||
    false
