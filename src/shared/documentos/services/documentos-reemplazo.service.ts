...
export class DocumentosReemplazoService {
  ...
  static async reemplazarArchivoSeguro(
    ...
    const { bucket, tabla, campoEntidad } = config

    // Variables para rollback
    let backupPath: string | null = null
    let archivoReemplazado = false
    let documento: DocumentoRegistroComun | null = null
    ...
    try {
      ...
      const { data: usuarioResult, error: usuarioError } = await supabase
        ...
        .eq('id', user.id)
        .single()
      const usuario = usuarioResult as unknown as { rol: string } | null
      ...
      if (passwordError) {
        logger.error(
          '❌ [REEMPLAZO] Error validando contraseña:',
          passwordError
        )
      ...
      const { data: _docData, error: fetchError } = await supabase
        ...
        .eq('id', documentoId)
        .single()
      documento = _docData as unknown as DocumentoRegistroComun

      if (fetchError || !documento) {
        throw new Error(`Documento no encontrado en ${tabla}`)
      }

      const entidadId = documento[campoEntidad] as string
      if (!entidadId) {
      ...
      const { data: archivoOriginal, error: downloadError } =
        await supabase.storage.from(bucket).download(documento.url_storage)

      if (downloadError || !archivoOriginal) {
        throw new Error(
          `No se pudo descargar archivo original: ${downloadError?.message}`
        )
      }
      ...
      if (archivoReemplazado && backupPath) {
        ...
        try {
          ...
          if (backupData && documento) {
            // Restaurar archivo original
            await supabase.storage
              .from(bucket)
              .update(documento.url_storage, backupData, {
                contentType: documento.tipo_mime as string,
                upsert: true,
              })
          }
        } catch (rollbackError) {
          logger.error(
            '💥 [ROLLBACK] Error crítico al restaurar archivo:',
            rollbackError
          )
          logger.error(
            `⚠️ ACCIÓN MANUAL REQUERIDA: Restaurar desde backup: ${backupPath}`
          )
        }
      }
  ...
  private static async registrarAuditoriaReemplazo(params: {
    documento: DocumentoRegistroComun
    nuevoArchivo: File
    motivo: string
    backupPath: string
    entidadId: string
    ...
    try {
      ...
      logger.error(
        '⚠️ [AUDITORÍA] Error al registrar (no crítico):',
        auditError
      )
    }
  }
