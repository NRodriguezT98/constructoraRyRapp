...
interface UseArchivarDocumentoModalProps {
  ...
  ) => void | Promise<void> // ✅ motivoDetalle ahora obligatorio
  onClose: () => void
}

export function useArchivarDocumentoModal({
  onConfirm,
  onClose,
}: UseArchivarDocumentoModalProps) {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Error al archivar documento'
      )
    } finally {
      setProcesando(false)
    }
