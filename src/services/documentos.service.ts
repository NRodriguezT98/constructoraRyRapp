...
export class DocumentosService {
    ...
    /**
     * Obtener URL pública temporal para descargar/visualizar
     */
     ...
     */
    static async eliminarDocumentoPermanente(
        documentoId: string
    ): Promise<void> {
        // 1. Obtener documento
    ...
    static async actualizarDocumento(
        documentoId: string,
        updates: Partial<Pick<DocumentoProyecto, 'titulo' | 'descripcion' | 'categoria_id' | 'etiquetas' | 'fecha_documento' | 'fecha_vencimiento' | 'es_importante' | 'metadata'>>
    ): Promise<DocumentoProyecto> {
        const { data, error } = await supabase
            .from('documentos_proyecto')
            .update(updates)
    ...
    static async buscarDocumentos(
        ...
        const { data, error } = await supabase
            ...
            .eq('proyecto_id', proyectoId)
            .eq('estado', 'activo')
            .or(`titulo.ilike.%${busqueda}%,nombre_original.ilike.%${busqueda}%,descripcion.ilike.%${busqueda}%`)
            .order('fecha_creacion', { ascending: false })
