...
import { ProyectosState, ProyectosActions, Proyecto, ProyectoFormData, FiltroProyecto, VistaProyecto } from '../types'
    ...
    devtools(
        persist(
            (set, get) => ({
                ...
                actualizarProyecto: async (id: string, data: Partial<ProyectoFormData>) => {
                    set({ cargando: true, error: undefined })
                    try {
                        const proyectoActualizado = await proyectosService.actualizarProyecto(id, data)
                        set(state => ({
                            proyectos: state.proyectos.map(p =>
                                p.id === id ? proyectoActualizado : p
                            ),
                            proyectoActual: state.proyectoActual?.id === id ? proyectoActualizado : state.proyectoActual,
                            cargando: false
                        }))
                ...
                eliminarProyecto: async (id: string) => {
                    ...
                    try {
                        ...
                        set(state => ({
                            ...
                            proyectoActual: state.proyectoActual?.id === id ? undefined : state.proyectoActual,
                            cargando: false
                        }))
                    } catch (error) {
