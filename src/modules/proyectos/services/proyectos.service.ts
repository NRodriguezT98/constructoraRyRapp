...
import type { Proyecto, ProyectoFormData, Manzana, EstadoManzana } from '../types'
...
class ProyectosService {
        ...
        if (formData.manzanas && formData.manzanas.length > 0) {
            ...
            if (errorManzanas) {
                ...
                manzanas = (manzanasCreadas || []).map(m => ({
                    ...
                    viviendasVendidas: 0,
                    precioBase: formData.manzanas.find(fm => fm.nombre === m.nombre)?.precioBase || 0,
                    superficieTotal: formData.manzanas.find(fm => fm.nombre === m.nombre)?.superficieTotal || 0,
    ...
    async actualizarProyecto(id: string, data: Partial<ProyectoFormData>): Promise<Proyecto> {
        ...
        if (data.fechaFinEstimada) updateData.fecha_fin_estimada = data.fechaFinEstimada
