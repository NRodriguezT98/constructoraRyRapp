...
export function ConfigurarPlanCredito({
  ...
  const [formCapital, setFormCapital] = useState(montoAprobado?.toString() ?? '')
  ...
  const handleCrearPlan = async () => {
    ...
    if (
      ...
      isNaN(tasaMensual) || tasaMensual <= 0 ||
      isNaN(numCuotas) || numCuotas < 1 ||
      !formFechaInicio
    ) return
  ...
  const formValido =
    !!formCapital && parseFloat(formCapital) > 0 &&
    !!formTasa && parseFloat(formTasa) > 0 &&
    !!formNumCuotas && parseInt(formNumCuotas, 10) >= 1 &&
    !!formFechaInicio
