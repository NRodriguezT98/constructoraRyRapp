        ...
        <table className='w-full text-sm'>
          ...
          <tbody className='divide-y divide-gray-100 dark:divide-gray-700'>
            {periodos.map(periodo => {
              const cfg =
                ESTADO_CONFIG[periodo.estado_periodo] ?? ESTADO_CONFIG.Futuro
      ...
      {hayAtrasados ? (
        <div className='flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 dark:border-red-800/50 dark:bg-red-900/10'>
          ...
          <p className='text-sm text-red-800 dark:text-red-200'>
            Hay períodos con déficit. Registra un abono para cubrir el atraso.
            La mora es una sugerencia que puedes incluir al registrar el abono.
          </p>
        </div>
      ) : null}
    </div>
