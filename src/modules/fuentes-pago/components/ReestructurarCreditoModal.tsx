...
export function ReestructurarCreditoModal({
  ...
  const preview = useMemo(() => {
    ...
    if (!capitalPendiente || !t || !c || isNaN(t) || isNaN(c) || c <= 0 || t <= 0) return null
    try {
  ...
  return (
    <>
      {/* Backdrop */}
      <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" onClick={onCerrar} aria-hidden="true" />
        ...
        <div className="w-full rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl dark:bg-gray-900">
          ...
          <div className="flex items-center justify-between rounded-t-2xl bg-gradient-to-r from-indigo-600 to-purple-600 px-5 py-4">
            <div className="flex items-center gap-2 text-white">
              ...
              <h2 id="modal-reestructurar-title" className="text-base font-bold">
          ...
          <div className="space-y-4 p-5">
            ...
            <div className="rounded-xl bg-gray-50 p-4 dark:bg-gray-800">
              ...
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Capital original</p>
                  <p className="font-medium text-gray-900 dark:text-white">
                    ${creditoActual.capital.toLocaleString('es-CO')}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Tasa mensual</p>
                  <p className="font-medium text-gray-900 dark:text-white">{creditoActual.tasa_mensual}%</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Cuotas pend.</p>
                  <p className="font-medium text-indigo-600 dark:text-indigo-400">{cuotasPendientes} de {creditoActual.num_cuotas}</p>
                </div>
              </div>
            ...
            {preview ? (
              <div className="rounded-xl bg-indigo-50 px-4 py-3 dark:bg-indigo-900/10">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Nueva cuota mensual:</span>
                  <span className="text-xl font-bold text-indigo-700 dark:text-indigo-300">
                    ${cuotaMensual.toLocaleString('es-CO')}
                  </span>
                </div>
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Capital pendiente: ${capitalPendiente.toLocaleString('es-CO')} · {nuevasCuotas} cuotas · {nuevaTasa}% mensual
                </p>
              </div>
            ) : (
