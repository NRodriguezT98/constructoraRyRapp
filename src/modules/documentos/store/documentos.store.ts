...
import type { DocumentoProyecto, CategoriaDocumento } from '../../../types/documento.types'
...
export const useDocumentosStore = create<DocumentosState>((set, get) => ({
    ...
    subirDocumento: async (params: any, userId: string) => {
        ...
        try {
            const nuevoDocumento = await DocumentosService.subirDocumento(params, userId)
            set((state) => ({
    ...
    crearCategoria: async (userId: string, categoria: any) => {
        try {
            const nuevaCategoria = await CategoriasService.crearCategoria(userId, categoria)
            set((state) => ({
...
export const selectDocumentosFiltrados = (state: DocumentosState) => {
    ...
    if (state.categoriaFiltro) {
        documentos = documentos.filter((doc) => doc.categoria_id === state.categoriaFiltro)
    }
...
export const selectEstadisticas = (state: DocumentosState) => {
    ...
    const porVencer = state.documentos.filter((doc) => {
        ...
        const hoy = new Date()
        const diasDiff = (fechaVenc.getTime() - hoy.getTime()) / (1000 * 60 * 60 * 24)
