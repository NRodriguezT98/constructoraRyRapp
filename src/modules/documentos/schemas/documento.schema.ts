...
export const categoriaFormSchema = z.object({
    ...
    icono: z.string()
        .min(1, 'Debe seleccionar un ícono')
        .default('Folder'),
})
...
export const documentoFormSchema = z.object({
    ...
    etiquetas: z.array(
        z.string()
            ...
            .regex(/^[a-zA-Z0-9\-_áéíóúñÁÉÍÓÚÑ\s]+$/, 'Etiqueta con caracteres inválidos')
            .trim()
    )
...
export const archivoSchema = z.object({
    archivo: z.instanceof(File, { message: 'Debe seleccionar un archivo' })
        .refine(
...
export const documentoConArchivoSchema = documentoFormSchema.merge(archivoSchema)

export type DocumentoConArchivoFormData = z.infer<typeof documentoConArchivoSchema>

// ============================================
// Helpers de Validación
// ============================================

export function validarNombreArchivo(nombre: string): { valido: boolean; error?: string } {
...
export function validarExtensionArchivo(nombre: string, tipoMime: string): { valido: boolean; error?: string } {
    ...
    if (partes.length > 2) {
        const extensionesPeligrosas = ['exe', 'bat', 'cmd', 'sh', 'ps1', 'vbs', 'js', 'msi']
