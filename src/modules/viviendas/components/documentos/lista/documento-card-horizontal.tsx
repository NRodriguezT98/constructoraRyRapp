...
import {
    ...
    formatFileSize,
    getFileExtension,
} from '@/modules/viviendas/types/documento-vivienda.types'
import {
...
interface DocumentoCardHorizontalProps {
  ...
  onView: (documento: DocumentoVivienda) => void
  onDownload: (documento: DocumentoVivienda) => void
  onToggleImportante: (documento: DocumentoVivienda) => void
  onArchive: (documento: DocumentoVivienda) => void
  onDelete: (documento: DocumentoVivienda) => void
  onRename?: (documento: DocumentoVivienda) => void
  onAsignarCategoria?: (documento: DocumentoVivienda) => void
  onRefresh?: () => void | Promise<void> // ðŸ†• Callback para refrescar despuÃ©s de versión
}
...
export function DocumentoCardHorizontal({
  ...
  const esDocumentoDeProceso = documento.etiquetas?.some(
    etiqueta => etiqueta.toLowerCase() === 'proceso' || etiqueta.toLowerCase() === 'negociaciÃ³n'
  )

  //  Solo mostrar "Ver Historial" si hay MÃS de 1 versión
  ...
  return (
    ...
    >
      {/* SECCIÃ“N IZQUIERDA: Icono de categoría compacto */}
      ...
      {/* SECCIÃ“N CENTRO: información principal */}
        ...
        <div className='relative z-30' ref={menuRef}>
          ...
          <AnimatePresence>
            {menuAbierto && (
              ...
              {/* BotÃ³n Ver Historial - si tiene versiones */}
              {tieneVersiones && (
                <button
                  type="button"
                  onClick={(e) => {
              ...
              {/* Separador - SecciÃ³n de versionado */}
              {esDocumentoDeProceso && (
                <div className='my-2 border-t border-gray-200 dark:border-gray-700' />
              )}

              {/* BotÃ³n Nueva versión - siempre visible para proyectos */}
              {esDocumentoDeProceso && (
                <button
                  type="button"
                  onClick={(e) => {
      ...
      <DocumentoVersionesModal
        ...
        onVersionRestaurada={async () => {
          cerrarModalVersiones()
          // ðŸ†• Refrescar lista de documentos
          await onRefresh?.()
        }}
      />
