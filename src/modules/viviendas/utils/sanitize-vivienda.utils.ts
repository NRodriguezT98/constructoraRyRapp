 ...
 */
export function sanitizeViviendaFormData(
  datos: ViviendaFormData
): ViviendaFormData {
  return {
    ...datos,
...
export function sanitizeViviendaUpdate(
  ...
  if ('proyecto_id' in datos && datos.proyecto_id)
    sanitized.proyecto_id = datos.proyecto_id
  if ('manzana_id' in datos && datos.manzana_id)
    sanitized.manzana_id = datos.manzana_id
  if ('numero' in datos) sanitized.numero = sanitizeString(datos.numero) || ''

  if ('lindero_norte' in datos)
    sanitized.lindero_norte = sanitizeString(datos.lindero_norte) ?? undefined
  if ('lindero_sur' in datos)
    sanitized.lindero_sur = sanitizeString(datos.lindero_sur) ?? undefined
  if ('lindero_oriente' in datos)
    sanitized.lindero_oriente =
      sanitizeString(datos.lindero_oriente) ?? undefined
  if ('lindero_occidente' in datos)
    sanitized.lindero_occidente =
      sanitizeString(datos.lindero_occidente) ?? undefined

  if ('matricula_inmobiliaria' in datos) {
    sanitized.matricula_inmobiliaria =
      sanitizeString(datos.matricula_inmobiliaria) ?? undefined
  }
  ...
  if ('area_construida' in datos)
    sanitized.area_construida = datos.area_construida
  ...
  if ('recargo_esquinera' in datos)
    sanitized.recargo_esquinera = datos.recargo_esquinera
