...
function parseUsuarioLabel(raw: string | null): {
  nombre: string
  rol: string | null
} {
  ...
  const cascadeActions = [
    {
      ...
      text: 'Negociación cerrada por renuncia',
    },
    {
      icon: Home,
      color: 'bg-amber-100 dark:bg-amber-900/30 text-amber-600',
      text: 'Vivienda liberada (estado → Disponible)',
    },
    {
      icon: UserX,
      color: 'bg-purple-100 dark:bg-purple-900/30 text-purple-600',
      text: 'Cliente marcado como "Renunció"',
    },
    {
      icon: CreditCard,
      color: 'bg-blue-100 dark:bg-blue-900/30 text-blue-600',
      text: 'Fuentes de pago inactivadas',
    },
  ]
      ...
      <div>
        ...
        <div className={styles.auditoria.card}>
          <div className={styles.auditoria.row}>
            <div
              className={`${styles.auditoria.rowIcon} bg-red-100 dark:bg-red-900/30`}
            >
              <User className='h-4 w-4 text-red-600' />
            </div>
            <div className={styles.auditoria.rowContent}>
              <p className={styles.auditoria.rowLabel}>Registrado por</p>
              <p className={styles.auditoria.rowValue}>
                {parseUsuarioLabel(renuncia.usuario_registro).nombre}
              </p>
          ...
          <div className={styles.auditoria.row}>
            <div
              className={`${styles.auditoria.rowIcon} bg-rose-100 dark:bg-rose-900/30`}
            >
            ...
            <div className={styles.auditoria.rowContent}>
              ...
              <p className={styles.auditoria.rowValue}>
                {formatDateTimeWithSeconds(renuncia.fecha_renuncia)}
              </p>
            </div>
          </div>
          {renuncia.fecha_creacion ? (
            <div className={styles.auditoria.row}>
              <div
                className={`${styles.auditoria.rowIcon} bg-gray-100 dark:bg-gray-800`}
              >
                <Calendar className='h-4 w-4 text-gray-500' />
              </div>
              <div className={styles.auditoria.rowContent}>
                <p className={styles.auditoria.rowLabel}>Creado en sistema</p>
                <p className={styles.auditoria.rowValueMuted}>
        ...
        <div>
          ...
          <div className={styles.auditoria.card}>
            {renuncia.usuario_cierre ? (
              <div className={styles.auditoria.row}>
                <div
                  className={`${styles.auditoria.rowIcon} bg-emerald-100 dark:bg-emerald-900/30`}
                >
                ...
                <div className={styles.auditoria.rowContent}>
                  ...
                  <p className={styles.auditoria.rowValue}>
                    {parseUsuarioLabel(renuncia.usuario_cierre).nombre}
                  </p>
            ...
            {renuncia.metodo_devolucion ? (
              <div className={styles.auditoria.row}>
                <div
                  className={`${styles.auditoria.rowIcon} bg-blue-100 dark:bg-blue-900/30`}
                >
                  <CreditCard className='h-4 w-4 text-blue-600' />
                </div>
                <div className={styles.auditoria.rowContent}>
                  <p className={styles.auditoria.rowLabel}>
                  ...
                  {renuncia.numero_comprobante ? (
                    <p className={styles.auditoria.rowValueMuted}>
                      Comprobante: {renuncia.numero_comprobante}
                    </p>
                  ) : null}
                </div>
              </div>
        ...
        <div className={styles.auditoria.cascadeList}>
          {cascadeActions.map((action, idx) => (
            <div key={idx} className={styles.auditoria.cascadeItem}>
              <div
                className={`${styles.auditoria.cascadeIcon} ${action.color}`}
              >
