  ...
  const metricas = [
    {
      ...
      valor: formatCOP(resumenFinanciero.valorNegociado),
      bg: 'bg-blue-50 dark:bg-blue-950/20',
      color: 'text-blue-700 dark:text-blue-400',
    },
    {
      label: 'Total abonado',
      valor: formatCOP(resumenFinanciero.totalAbonado),
      bg: 'bg-emerald-50 dark:bg-emerald-950/20',
      color: 'text-emerald-700 dark:text-emerald-400',
    },
    {
      label: 'Saldo pendiente',
      valor: formatCOP(resumenFinanciero.saldoPendiente),
      bg: 'bg-amber-50 dark:bg-amber-950/20',
      color: 'text-amber-700 dark:text-amber-400',
    },
    {
      label: 'Porcentaje pagado',
      valor: `${resumenFinanciero.porcentajePagado.toFixed(1)}%`,
      ...
      <div className={styles.financiero.metricsGrid}>
        {metricas.map(({ label, valor, bg, color }) => (
          <div key={label} className={`${styles.financiero.metricCard} ${bg}`}>
            <p className={`${styles.financiero.metricValue} ${color}`}>
              {valor}
            </p>
          ...
          <div className='mt-2 grid grid-cols-2 gap-3 sm:grid-cols-4'>
            <div>
              ...
              <p className='text-sm font-bold text-gray-900 dark:text-white'>
                {resumenFinanciero.descuento.tipo}
              </p>
            </div>
            <div>
          ...
          <div className='mt-2 grid grid-cols-2 gap-3'>
            <div>
              ...
              <p className='text-sm font-bold text-red-700 dark:text-red-400'>
                {formatCOP(resumenFinanciero.retencion.monto)}
              </p>
            </div>
            <div>
              <p className='text-xs text-gray-500 dark:text-gray-400'>Motivo</p>
              <p className='text-sm text-gray-700 dark:text-gray-300'>
                {resumenFinanciero.retencion.motivo}
              </p>
            </div>
          </div>
        </div>
      ...
      {(() => {
        ...
        return (
          <div
            className={
              devolucionEfectuada
                ? 'rounded-xl border-2 border-emerald-400 bg-emerald-50/80 p-5 dark:border-emerald-600 dark:bg-emerald-950/30'
          ...
          >
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className={styles.financiero.devolucionMonto}>
                  {formatCOP(resumenFinanciero.montoADevolver)}
                </p>
              </div>
              <div className='text-right'>
                {devolucionEfectuada ? (
                  <span className='inline-flex items-center gap-1.5 rounded-full border border-emerald-300 bg-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-700 dark:border-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-400'>
                    <CheckCircle2 className='h-3.5 w-3.5' />
                    Devolución efectuada
                  </span>
                ) : (
                  <p className='text-xs text-gray-500 dark:text-gray-400'>
                    {renuncia.requiere_devolucion
                      ? 'Requiere devolución'
                      : 'Sin devolución requerida'}
                  </p>
                )}
              </div>
            </div>
            ...
            (renuncia.metodo_devolucion ||
              ...
              <div className='mt-3 grid grid-cols-1 gap-2 border-t border-emerald-200 pt-3 text-xs dark:border-emerald-800 sm:grid-cols-3'>
                {renuncia.fecha_devolucion ? (
                  <div>
                    ...
                    <p className='font-semibold text-gray-800 dark:text-gray-200'>
                      {formatDateTimeWithSeconds(renuncia.fecha_devolucion)}
                    </p>
                  </div>
                ) : null}
                {renuncia.metodo_devolucion ? (
      ...
      negociacion.promesa_firmada_url ? (
        <div>
          ...
          <div className='mt-2 space-y-2'>
            {negociacion.promesa_compraventa_url ? (
              <a
                href={negociacion.promesa_compraventa_url}
                target='_blank'
                rel='noopener noreferrer'
                className={styles.financiero.docLink}
              >
            ...
            {negociacion.promesa_firmada_url ? (
              <a
                href={negociacion.promesa_firmada_url}
                target='_blank'
                rel='noopener noreferrer'
                className={styles.financiero.docLink}
              >
