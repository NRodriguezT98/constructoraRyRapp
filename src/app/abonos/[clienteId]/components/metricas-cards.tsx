  ...
  return (
    <div className="space-y-3">
      ...
      {tieneIntereses ? (
        ...
        >
          ...
          <p className="text-xs leading-relaxed text-violet-800 dark:text-violet-200">
            <span className="font-semibold">Financiamiento con la constructora.</span>{' '}
            Este cliente tiene un crédito directo que genera intereses por{' '}
            <span className="font-bold text-violet-600 dark:text-violet-300">{formatCurrency(metricas.interesesTotales)}</span>.{' '}
            Precio de vivienda:{' '}
            <span className="font-semibold">{formatCurrency(metricas.valorVivienda)}</span>{' '}
            — Total con intereses:{' '}
            <span className="font-semibold">{formatCurrency(metricas.totalComprometido)}</span>.
          </p>
        </motion.div>
      ) : null}
      ...
      <motion.div
        ...
        className="rounded-2xl border border-gray-200 dark:border-white/8 bg-white dark:bg-white/[0.06] shadow-sm dark:shadow-none backdrop-blur overflow-hidden"
      >
        <div className="flex flex-col lg:flex-row">
          {tieneIntereses ? (
            /* ── CON FINANCIAMIENTO: grid 2×2 (4 métricas) ── */
            <div className="flex-1 grid grid-cols-2 divide-x divide-y divide-gray-100 dark:divide-white/5">
              <MetricaItem
                Icon={Home}
            ...
            <div className="flex-1 grid grid-cols-3 divide-x divide-gray-100 dark:divide-white/5">
              <MetricaItem
                Icon={Home}
                gradient="from-blue-500 to-indigo-600"
                label="Valor Vivienda"
          ...
          <div className="flex items-center justify-center border-t lg:border-t-0 lg:border-l border-gray-100 dark:border-white/5 px-6 py-5 lg:py-0 lg:min-w-[140px]">
            <motion.div
              ...
              className="relative"
            >
              <svg width="100" height="100" viewBox="0 0 100 100" style={{ filter: `drop-shadow(0 0 10px ${ringColor}50)` }}>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center leading-none">
                <span className="text-2xl font-extrabold text-gray-900 dark:text-white">{pct.toFixed(0)}%</span>
                <span className="text-[10px] text-gray-400 dark:text-white/40 mt-0.5 uppercase tracking-wider font-medium">pagado</span>
              </div>
            </motion.div>
          </div>
        </div>
...
function MetricaItem({ Icon, gradient, label, subtitle, value, valueColor, delay, highlight }: MetricaItemProps) {
  return (
    <motion.div
    ...
    >
      <div className="flex items-center gap-2.5 mb-1.5">
        <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${gradient} flex items-center justify-center shadow-md flex-shrink-0`}>
          <Icon className="w-4 h-4 text-white" />
        </div>
        <div>
          <p className="text-xs font-medium text-gray-500 dark:text-white/50 leading-tight">{label}</p>
          <p className="text-[11px] text-gray-400 dark:text-white/30 leading-tight">{subtitle}</p>
        </div>
      </div>
      <p className={`text-xl font-extrabold leading-tight ${valueColor}`}>{value}</p>
    </motion.div>
  )
}
