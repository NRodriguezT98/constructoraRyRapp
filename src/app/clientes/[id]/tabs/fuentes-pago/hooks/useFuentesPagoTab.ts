...
export function useFuentesPagoTab({ cliente }: UseFuentesPagoTabProps) {
  ...
  const [fuenteParaEditar, setFuenteParaEditar] = useState<FuentePago | null>(
    null
  )
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          `
          id, valor_negociado, valor_total, valor_total_pagar, estado,
          vivienda_id,
          viviendas!negociaciones_vivienda_id_fkey(
            valor_base, numero, estado, recargo_esquinera, gastos_notariales,
            manzanas!viviendas_manzana_id_fkey(
              nombre,
              proyectos!manzanas_proyecto_id_fkey(nombre)
            )
          )
        `
        )
  ...
  const negociacionId = negociacion?.id ?? null
  ...
  const {
    ...
    queryFn: () => {
      if (!negociacionId) {
        throw new Error('Negociación requerida')
      }

      return fuentesPagoService.obtenerFuentesPagoNegociacion(negociacionId)
    },
  ...
  const { data: tiposFuentesConfig = [], isLoading: isLoadingTipos } = useQuery(
    {
  ...
  const { data: requisitosConfig = [], isLoading: isLoadingRequisitos } =
    useQuery({
      ...
      queryFn: async () => {
        const { data, error } = await supabase
          ...
          .select(
            'id,tipo_fuente,paso_identificador,titulo,descripcion,instrucciones,nivel_validacion,tipo_documento_sugerido,categoria_documento,alcance,orden,activo,version,fecha_creacion,fecha_actualizacion'
          )
          .eq('activo', true)
  ...
  const crearFuenteMutation = useMutation({
    mutationFn: (tipo: TipoFuentePago) => {
      ...
      return fuentesPagoService.crearFuentePago({
  ...
  const getColorClasses = useCallback((color: string) => {
    const colorMap = {
      ...
      indigo:
        'bg-gradient-to-br from-indigo-500 to-indigo-600 shadow-indigo-500/50',
      yellow:
  ...
  const getTipoConfig = useCallback(
    (tipo: TipoFuentePago) => {
  ...
  const getDocumentosRequeridos = useCallback(
    (tipoFuente: string | undefined) => {
  ...
  const getDocumentacionEstado = useCallback(
    (fuente: FuentePago) => {
      ...
      const documentosSubidos =
        documentosCliente?.filter(doc =>
          documentosRequeridos.some(
            reqDoc =>
              doc.titulo?.toLowerCase().includes(reqDoc.titulo.toLowerCase()) ||
              doc.tipo_documento
                ?.toLowerCase()
                .includes(reqDoc.titulo.toLowerCase())
          )
        ) || []
      ...
      const documentosPendientes = documentosRequeridos.filter(
        doc =>
          !documentosSubidos.some(
            subido =>
              (subido.titulo?.toLowerCase() || '').includes(
                doc.titulo.toLowerCase()
              ) ||
              (subido.tipo_documento?.toLowerCase() || '').includes(
                doc.titulo.toLowerCase()
              )
          )
      )
      ...
      return {
        ...
        documentos: {
          ...
          obligatoriosPendientes: documentosPendientes.filter(
            d => d.obligatorio
          ).length,
  ...
  const todasFuentesCreadas = useMemo(() => {
    ...
    return tiposDisponibles.every(tipoId =>
      tiposCreados.has(tipoId as TipoFuentePago)
    )
  ...
  const tiposDisponibles = useMemo(() => {
    ...
    const tiposCreados = new Set(fuentesPago.map(f => f.tipo).filter(Boolean))
    return tiposFuentesConfig.filter(
      tipo => !tiposCreados.has(tipo.nombre as TipoFuentePago)
    )
  }, [tiposFuentesConfig, fuentesPago])
  ...
  const valorTotalVivienda = useMemo(() => {
    ...
    if (negociacion?.valor_total_pagar) {
      valor = Number(negociacion.valor_total_pagar)
  ...
  const estadoValidacion = useMemo((): EstadoValidacion => {
    ...
    if (tieneFuentes && metricas.saldoPendiente > 100) {
      mensajes.push(
        `💰 Faltan ${formatCurrency(metricas.saldoPendiente)} por cubrir`
      )
    }

    if (tieneFuentes && metricas.saldoPendiente < -100) {
      mensajes.push(
        `📈 Excede por ${formatCurrency(Math.abs(metricas.saldoPendiente))}`
      )
    }

    if (tieneFuentes && !todasConDocumentacion) {
      mensajes.push(
        `📄 ${metricas.fuentesSinDocumentacion} fuentes sin documentación`
      )
    }

    const puedeRegistrarAbonos =
      tieneFuentes && sumaCuadra && todasConDocumentacion
  ...
  const handleGuardarFuente = useCallback(
    async (fuenteEditada: FuenteEditada) => {
  ...
  const handleAgregarFuente = useCallback(
    async (tipo: TipoFuentePago) => {
  ...
  const handleEliminarFuente = useCallback(
    async (id: string) => {
      const confirmado = await confirm({
        title: 'Eliminar fuente de pago',
        message:
          '¿Estás seguro de eliminar esta fuente de pago? Esta acción no se puede deshacer.',
        variant: 'danger',
    ...
    [eliminarFuenteMutation, confirm]
  )
  ...
  const handleGestionarDocumentos = useCallback((_fuenteId: string) => {
    toast.info(
      'Navegando al módulo de Documentos para gestionar documentación...'
    )
    // TODO: Implementar navegación real al módulo de documentos
    // router.push(`/documentos?fuente=${fuenteId}&action=upload`)
  }, [])

  const handleVerDocumentos = useCallback((_fuenteId: string) => {
  ...
  const isLoading =
    isLoadingNegociacion ||
    isLoadingFuentes ||
    isLoadingTipos ||
    isLoadingRequisitos
  const isUpdating =
    actualizarFuenteMutation.isPending ||
