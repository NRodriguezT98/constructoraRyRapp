...
export function ProgressBarProminente({
  ...
  const porcentajeAbonado = valorTotal > 0 ? (totalAbonado / valorTotal) * 100 : 0
  const porcentajeFuentes = valorTotal > 0 ? (totalFuentesPago / valorTotal) * 100 : 0
      ...
      <div className="flex items-center justify-between">
        ...
        {diasDesdeUltimoAbono !== null && (
          <div className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400">
            ...
            Último abono hace {diasDesdeUltimoAbono} día{diasDesdeUltimoAbono !== 1 ? 's' : ''}
          </div>
        )}
      </div>

      {/* Progress Bars Container */}
      <div className="p-4 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800/50 dark:to-gray-900/50 border border-gray-200 dark:border-gray-700/50 space-y-3">
        {/* Abonos Progress */}
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-bold text-green-600 dark:text-green-400">
              ${totalAbonado.toLocaleString('es-CO')} ({porcentajeAbonado.toFixed(1)}%)
            </span>
          </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="font-bold text-blue-600 dark:text-blue-400">
              ${totalFuentesPago.toLocaleString('es-CO')} ({porcentajeFuentes.toFixed(1)}%)
            </span>
          </div>
          <div className="h-3 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
            <motion.div
