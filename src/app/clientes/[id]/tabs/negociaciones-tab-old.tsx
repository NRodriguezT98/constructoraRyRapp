 ...
 * Tab de Negociaciones - Refactorizado
 *
 * SEPARACIÓN DE RESPONSABILIDADES:
 * - TODA la lógica está en useNegociacionesTab hook
...
import { useNegociacionesTab } from '@/modules/clientes/hooks'
...
export function NegociacionesTab({ cliente }: NegociacionesTabProps) {
  ...
  const {
    ...
    volverALista,
    totales,
  } = useNegociacionesTab({ clienteId: cliente.id })
  ...
  // ✅ Verificar documento de identidad desde sistema moderno
  const { tieneCedula } = useDocumentoIdentidad({ clienteId: cliente.id })
            ...
            >
              ...
              <div className="mb-3 flex items-start justify-between">
                ...
                <span className={`inline-flex items-center gap-1 rounded-full ${estadoConfig.bg} ${estadoConfig.text} px-2.5 py-1 text-[10px] font-semibold`}>
                  {/* Renderizar icono directamente según estado */}
                  {negociacion.estado === 'Activa' && <CheckCircle2 className="h-3 w-3" />}
                  {negociacion.estado === 'Suspendida' && <Clock className="h-3 w-3" />}
                  {negociacion.estado === 'Cerrada por Renuncia' && <XCircle className="h-3 w-3" />}
                  {negociacion.estado === 'Completada' && <Building2 className="h-3 w-3" />}
                  {negociacion.estado}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between border-t border-gray-200 pt-3 dark:border-gray-700">
                ...
                <div className="flex items-center gap-2">
                  <Tooltip content="Ver historial de versiones y modificaciones" side="top">
                    <button
                      onClick={() => {
                        setNegociacionSeleccionada(negociacion.id)
                        setShowHistorial(true)
                      }}
                      className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white shadow-md transition-all hover:bg-blue-700 hover:shadow-lg"
                    >
          ...
          <Tooltip content="Crear nueva negociación" side="left">
            <motion.button
              ...
              onClick={() => {
                ...
                router.push(`/clientes/${clienteSlug}/asignar-vivienda?nombre=${encodeURIComponent(cliente.nombre_completo || cliente.nombres || '')}` as any)
              }}
